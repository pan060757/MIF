...
#####住院数据预处理
###（个人编号 1,(医院等级 5，住院天数，药品费 10,起付线 14,报销比例 15，统筹账户支付 17，出院病种编号 26(以编号前3位为准))))）
def hospitalProcessing(line):
    ...
    if line[21][-2:]=='15':
        ...
        if(line[21]!="" and line[22]!=""):       ###可能存在未记录出院时间和住院时间的住院记录
            ...
            days = (d2 - d1).days
            return (line[1],(line[5],days,line[10],line[14],line[15],line[17],line[26][0:3]))
        else:
            return (str(999999), 1)
    else:
        return (str(999999),1)
...
def chargeProcessing(line):
    line = line.encode("utf-8").split(",")
    if line[2][0:4] == '2015'and line[3]=='310':
        if line[5]=="":
            line[5]='0'
...
####计算年龄
#####(个人编号,(身份,性质，年龄,性别,月度工资，医院等级,住院天数,药品费，起付线，报销比例，出院病种,统筹账户支付))
def ageComputed((key, value)):
    ...
    elif wage*12>=41121 and wage*12<=3*41121:
    ...
    if value[3]=='1':
        ...
        sex='woman'
    return (key, (value[0], value[1],ageGroup,sex, wageLevel, value[5], value[6],
...
#####(个人编号,(身份,性质，年龄,性别,年度工资，医院等级,出院病种))
out = open('output/fpm_data.txt', 'w+')
for (key,value) in result.collect():
  out.write("%s,%s,%s,%s,%s,%s,%s,%s\n"%(key,value[0],value[1],value[2],value[3],value[4],value[5],value[11]))
out.close()
